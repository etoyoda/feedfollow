...
# バックアップ済みアーカイブの自動削除ツール。
#
# Apache アクセスログから、登録済みアーカイブホストによる
# 正常な GET を検出し、バックアップ済みと判断できる
# /nwp/a* 配下のアーカイブファイルを削除する。
#
# 不完全転送や未登録ホストによるアクセスでは削除を行わない。
# 外部ストレージへの保管確認後にローカルディスク使用量を抑える
# ことを目的とする。
